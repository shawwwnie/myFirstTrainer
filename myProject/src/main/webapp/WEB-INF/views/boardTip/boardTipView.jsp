    ...
    $.ajax({
        ...
        success : function(data){			
            ...
            $.each(data, function(key, value){
                ...
                a += '<div class="replyComment'+value.freec_num+'"> <a onclick="select_replyComment('+ value.free_num +',' + value.freec_num + ')"> 댓글 '+value.countComment+'</a></div>'
				a += '</div>';
            });		
...
/* 댓글의댓글 글읽기 및 쓰기 */
function select_replyComment(free_num,freec_num){
	commentList();
 	 $.ajax({
        ...
        success : function(data){					
            ...
            $.each(data, function(key, value){
	            ...
	            a += '|&nbsp;<a onclick="delete_replyComment('+ value.free_num +',' + value.rfreec_num +','+ freec_num + ');">삭제</a>';
	            }
	            a += '</div>';
            });
			a += '<input type="text" id="replyComment_content" style="padding-left: 10px;" placeholder="답댓글을 입력해주세요">';
			a += '<input type="button" value="등록" onclick="submit_replyComment('+free_num+','+freec_num+');">';
...
/* 댓글의 댓글 삭제하기 */
function delete_replyComment(free_num,rfreec_num,freec_num){
